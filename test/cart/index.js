...
describe("Shopping Cart tests", () => {
  ...
  before(async () => {
    ...
    const {
      body: { cart_id }
    } = await request(server).get(`${BASE_URL}/generateUniqueId`);
    cartId = cart_id;
  });
  ...
  describe("Shopping cart", () => {
    ...
    it("should get all items in the cart", async () => {
      const { body } = await request(server).get(`${BASE_URL}/${cartId}`);
      expect(body).to.be.an("array");
    });
    ...
    it("should return total amount from cart", async () => {
      const {
        body: { total_amount }
      } = await request(server).get(`${BASE_URL}/totalAmount/${cartId}`);
      expect(total_amount).to.be.a("number");
    });

    it("should update an item in shopping cart", async () => {
      const res = await request(server).get(`${BASE_URL}/${cartId}`);
      const [item] = res.body;
      const { body } = await request(server)
    ...
    it("should get error if quantity is missing", async () => {
      const res = await request(server).get(`${BASE_URL}/${cartId}`);
      const [item] = res.body;
      const { status } = await request(server).put(`${BASE_URL}/update/${item.item_id}`);
      expect(status).to.equal(400);
    });
    ...
    it("should remove item from cart", async () => {
      ...
      const {
        body: { message }
      } = await request(server).delete(`${BASE_URL}/removeProduct/${item.item_id}`);
      expect(message).to.be.equal("item removed from cart");
    });

    it("should remove all items from the cart", async () => {
      const { body } = await request(server).delete(`${BASE_URL}/empty/${cartId}`);
