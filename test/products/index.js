...
describe("Product tests", () => {
  ...
  describe("Get /api/products", () => {
    ...
    it("should search products", async () => {
      const {
        body: { rows }
      } = await request(server).get(`${BASE_URL}/?query_string=Partridge`);
    ...
    it("should handle pagination", async () => {
      const {
        body: { rows }
      } = await request(server).get(`${BASE_URL}/?limit=2&order=name,ASC&page=1`);
  ...
  describe("Get /api/product/:product_id", () => {
    it("should retrieve a single product", async () => {
      const {
        body: { product_id }
      } = await request(server).get(`${BASE_URL}/91`);
  ...
  describe("Get /api/product/inCategory/:category_id", () => {
    it("should retrieve all products in a category", async () => {
      const {
        body: { rows }
      } = await request(server).get(`${BASE_URL}/inCategory/1/?limit=2&order=name,ASC&page=1`);
      expect(rows.length).to.equal(2);
    });
  ...
  describe("Get /api/product/inDepartment/:department_id", () => {
    it("should retrieve all products in a department", async () => {
      const {
        body: { rows }
      } = await request(server).get(`${BASE_URL}/inDepartment/1/?limit=2&order=name,ASC&page=1`);
      expect(rows.length).to.equal(2);
    });

    it("should return error if department_id is not a number", async () => {
      const { status } = await request(server).get(`${BASE_URL}/inDepartment/f`);
      expect(status).to.equal(400);
    });
  });
  ...
  describe("Get /api/product/:product_id/locations", () => {
    it("should get product locations", async () => {
      const {
        body: { category_id }
      } = await request(server).get(`${BASE_URL}/91/locations`);
