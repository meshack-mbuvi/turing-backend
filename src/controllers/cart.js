...
export class CartController {
  ...
  static async add(req, res) {
    const {
      body: { cart_id, product_id, attributes }
    } = req;
        ...
        include: [
          {
            model: Product,
            on: {
              id: Sequelize.where(Sequelize.col("Cart.product_id"), "=", Sequelize.col("Product.product_id"))
            }
          }
        ]
      });
  ...
  static async getCart(req, res) {
    ...
    return res.status(404).send({ error: handleCartErrors("CART_02", 404, "cart_id") });
  }
